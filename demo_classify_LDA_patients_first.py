...
def train_and_test_classifier(input_for_classifier, tuple_to_predict=[]):
    print("Training and testing Decision Tree classifier for", len(input_for_classifier.columns)-1,"topics")
    DTclassifier = ClassifierProcessor(doc_to_topic_matrix= input_for_classifier, unseen_doc_features= tuple_to_predict, classifier= 'DT')
    DTclassifier.train_classifier()

    print("Training and testing Gaussian Naive Bayes classifier for", len(input_for_classifier.columns)-1,"topics")
    GNBclassifier = ClassifierProcessor(doc_to_topic_matrix= input_for_classifier, unseen_doc_features= tuple_to_predict, classifier= 'GNB')
    GNBclassifier.train_classifier()

    print("Training and testing ANN classifier for", len(input_for_classifier.columns)-1,"topics")
    ANNclassifier = ClassifierProcessor(doc_to_topic_matrix= input_for_classifier, unseen_doc_features= tuple_to_predict, classifier= 'ANN')
    ANNclassifier.train_classifier()
